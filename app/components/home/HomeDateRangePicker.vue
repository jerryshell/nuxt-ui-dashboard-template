...
const subtractRange = (
  base: CalendarDate,
  range: { days?: number; months?: number; years?: number },
): CalendarDate => {
  if (range.days) return base.subtract({ days: range.days });
  if (range.months) return base.subtract({ months: range.months });
  if (range.years) return base.subtract({ years: range.years });
  return base;
};
...
const isRangeSelected = (range: { days?: number; months?: number; years?: number }) => {
  ...
  const startDate = subtractRange(currentDate, range);
